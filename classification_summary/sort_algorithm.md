...
排序算法的时间复杂度：
![sort_complexity](images/sort_complexity.jpg)
...
### 选择排序
选择排序每次将第i小的元素放到位置i处，可以将其看作冒泡排序的改进，冒泡排序是每次将第i小的元素交换到位置i，选择排序是直接找出这个元素并插入位置i。
```python
...
2. 合并：对于每一组小片段，将它们一一合并直到组合成完整序列。合并过程中完成对子序列的排序。
